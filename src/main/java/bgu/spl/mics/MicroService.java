...
public abstract class MicroService implements Runnable {
    ...
    ConcurrentHashMap<Class<? extends Request>, Callback> RequestsCallbacks;
    ConcurrentHashMap<Class<? extends Broadcast>, Callback> BroadcastCallbacks;
    ConcurrentHashMap<Request, Callback> awaitingComplitaion;
    ...
    public MicroService(String name) {
        ...
        RequestsCallbacks = new ConcurrentHashMap<Class<? extends Request>, Callback>();
        BroadcastCallbacks = new ConcurrentHashMap<Class<? extends Broadcast>, Callback>();
        awaitingComplitaion = new ConcurrentHashMap<Request, Callback>();
    }
    ...
    protected final <R extends Request> void subscribeRequest(Class<R> type, Callback<R> callback) {
    	RequestsCallbacks.putIfAbsent(type, callback);
    ...
    protected final <B extends Broadcast> void subscribeBroadcast(Class<B> type, Callback<B> callback) {
    	BroadcastCallbacks.putIfAbsent(type, callback);
    ...
    protected final <T> boolean sendRequest(Request<T> r, Callback<T> onComplete) {
    	if( MessageBusImpl.getInstance().sendRequest(r,this)){
    		awaitingComplitaion.putIfAbsent(r, onComplete);
    		return true;
    	}
    	return false;
    }
    ...
    public final void run() {
        ...
        while (!terminated) {
            try{
            	Message toHandle = MessageBusImpl.getInstance().awaitMessage(this);
            		if(toHandle.getClass()==RequestCompleted.class){
            			RequestCompleted r =(RequestCompleted) toHandle;
            			awaitingComplitaion.get(r.getCompletedRequest()).call(r.getResult());
            			awaitingComplitaion.remove(r.getCompletedRequest());
            		}
            		else if(toHandle instanceof Request){
            			RequestsCallbacks.get(toHandle.getClass()).call(toHandle);
            	    }
            		else{
            			Broadcast r = (Broadcast)  toHandle;
            			BroadcastCallbacks.get(r.getClass()).call(toHandle);
            		}
      ...
      /*  while(!awaitingComplitaion.isEmpty())
			try {
				synchronized(this){
				this.wait(300);
				}
